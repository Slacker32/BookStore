...
namespace BooksShopCore.WorkWithUi.LogicsSite.WorkWithBooks
{
    public class WorkWithBooks:IWorkWithBooks
    {
        ...
        private IDataRepositoryAsync<BookData> bookRepositoryAsync;
        private IDataRepositoryAsync<ExchangeRatesData> exchangeRatesRepositoryAsync;
        ...
        public WorkWithBooks()
        {
            ...
            exchangeRatesRepositoryAsync = new GenericRepositoryAsync<ExchangeRatesData, BookStoreContext>();
        }
        ...
        public async Task<IList<BookUi>> ShowAllBooksAsync(string languageCode = null, string currencyCode = null)
        {
            var ret = new List<BookUi>();
            try
            {
                ...
                ret = await MapBookDataAsync(booksListFromStorage, languageCode, currencyCode);
            }
            catch (Exception ex)
            {
                ex.Data.Add("ShowAllBooks", "Ошибка получения книг из хранилища данных");
        ...
        public async Task<IList<BookUi>> FindBooksAsync(string searchStr, string languageCode, string currencyCode)
        {
            var ret = new List<BookUi>();
            try
            {
                ...
                booksListFromStorage = booksListFromStorage.Where(
                            p => p.Authors.Count(p1 => p1.Name.IndexOf(searchStr, StringComparison.OrdinalIgnoreCase) >= 0) > 0 ||
                                 p.NameBooksTranslates.Count(p1 => p1.NameBook.IndexOf(searchStr, StringComparison.OrdinalIgnoreCase) >= 0) > 0).ToList();
        ...
        public async Task<BookUi> GetBookAsync(int index, string languageCode, string currencyCode)
        {
            BookUi ret = null;
            try
            {
                var booksListFromStorage = await bookRepositoryAsync.GetWithIncludeAsync(p => p.Id.Equals(index),
                    p => p.Authors, p => p.BooksStorages, p => p.NameBooksTranslates, p => p.PricePolicy, p => p.FormatBook,
                    p => p.NameBooksTranslates.Select(p1 => p1.Language), p => p.PricePolicy.Select(p1 => p1.Currency));

                var listBook = await MapBookDataAsync(booksListFromStorage, languageCode, currencyCode);
                ret= listBook.First();
            }
            catch (Exception ex)
            {
        ...
        private async Task<List<BookUi>> MapBookDataAsync(IList<BookData> booksListFromStorage, string languageCode = null, string currencyCode = null)
        {
            var ret = new List<BookUi>();
            if (booksListFromStorage != null)
            {
                foreach (var item in booksListFromStorage)
                {
                    ...
                    else
                    {
                        if (item.PricePolicy?.Count > 0)
                        {
                            var reCalcPrice = item.PricePolicy[0].Price;
                            var reCalcCurrencyCodeID = item.PricePolicy[0].CurrencyDataId;
                            var listData = await exchangeRatesRepositoryAsync.ReadAllAsync();
                            var rate= listData.FirstOrDefault(p => p.CurrencyDataFromId.Equals(reCalcCurrencyCodeID) && p.CurrencyTo.CurrencyCode.Equals(currencyCode, StringComparison.OrdinalIgnoreCase))?.Rate;
                    ...
                    {
                        Price = tempPrice,
                        Currency = await currencyStorage.ReadAsync(currencyCode)
                    }
                        );
                    //book.Price = tempPrice;
                    //book.Currency = CurrencyStorage.Read(currencyCode);
